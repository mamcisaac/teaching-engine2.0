...
export const API_BASE = process.env.TEST_SERVER_URL || process.env.API_BASE || 'http://127.0.0.1:3000';
...
async function waitForServices(page: Page, maxRetries = 60): Promise<void> {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (i % 10 === 0) { // Log every 10th attempt
        console.log(`Services status: API ${apiReady ? 'OK' : 'Failed'}, Frontend ${frontendReady ? 'OK' : 'Failed'}`);
      }
    } catch (error) {
      if (i % 10 === 0) { // Log every 10th attempt to reduce noise
        console.log(`Service check attempt ${i + 1} failed:`, error instanceof Error ? error.message : error);
      }
    }
  ...
  throw new Error(`Services did not become available within timeout. API: ${apiReady}, Frontend: ${frontendReady}`);
}
...
export class PlannerPageObject {
  ...
  async clickAutoFill(): Promise<void> {
    const autoFillButton = this.page.locator('text=Auto Fill');
  ...
  async expectActivityInDay(dayTestId: string, activityName: string, shouldExist = true): Promise<void> {
...
export class TestDataFactory {
  constructor(private page: Page, private token: string) {}
  ...
  async createMilestone(
    ...
    const milestoneData = {
      ...
      subjectId,
      startDate: (options.startDate || new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000)).toISOString(),
      endDate: (options.endDate || new Date(today.getTime() + 14 * 24 * 60 * 60 * 1000)).toISOString(),
    };
