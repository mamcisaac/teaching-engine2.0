...
async function fixNotreMonde() {
  console.log('🔧 FIXING "NOTRE MONDE EN CARTES" LESSONS');
  console.log('='.repeat(60));
  
  // Get all lessons from this unit
  const lessons = await prisma.eTFOLessonPlan.findMany({
    where: { 
      unitPlan: { title: 'Notre monde en cartes' }
    }
  });
  ...
  for (const lesson of lessons) {
    ...
    if (!lesson.extensions) {
      // Context-appropriate extensions for geography and mapping
      updateData.extensions = "Créer une carte 3D détaillée; Explorer avec Google Earth; Concevoir un atlas personnel illustré";
    }
    
    // Check if grouping is missing and add appropriate structure
    if (!lesson.grouping) {
      // Varied grouping for exploration and mapping activities
      updateData.grouping = "Exploration en grand groupe, cartographie en pairs, création individuelle";
    }
  ...
  console.log(`\n✨ Fixed ${fixedCount} lessons in "Notre monde en cartes"`);
...
fixNotreMonde().catch(console.error);