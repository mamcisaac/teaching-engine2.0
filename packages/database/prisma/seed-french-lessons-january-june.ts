    ...
    if (!winterUnit) {
      ...
      janDates.forEach((date, index) => {
        lessons.push({
          ...
          environmentalConnections: 'Winter wildlife care, energy conservation, seasonal changes',
    ...
    if (!springUnit) {
      ...
      springDates.forEach((date, index) => {
        lessons.push({
          ...
          isSubFriendly: true,
          environmentalConnections: `Spring focus: ${springTopics[index]} - outdoor learning opportunities`
        });
      });
    }
