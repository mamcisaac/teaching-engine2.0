...
async function seedHealthFPSComprehensive36Lessons() {
  ...
  try {
    ...
    lessons.push({
      ...
      crossCurricular: JSON.stringify(['Social Studies: community and identity', 'Arts: self-portraits', 'French: identity vocabulary']),
      vocabulary: JSON.stringify(['identité', 'unique', 'spécial', 'famille', 'culture', 'tradition', 'caractéristique', 'intérêt']),
      homeConnection: 'Family identity interview: Ask family members about traditions, heritage, and what makes your family special.',
      socialEmotionalLearning: 'Self-awareness, identity formation, celebrating diversity, belonging',
      isSubFriendly: true,
      unitPlanId: unit1.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['French: emotion vocabulary', 'Arts: emotion art', 'Drama: emotion role-play']),
      vocabulary: JSON.stringify(['émotion', 'sentiment', 'heureux', 'triste', 'fâché', 'peur', 'excité', 'calme', 'normal', 'expression']),
      homeConnection: 'Family feelings talk: Share the feelings journal with family. Practice "I feel..." statements at home.',
      socialEmotionalLearning: 'Emotion awareness, self-regulation, emotional vocabulary, emotional intelligence',
      isSubFriendly: true,
      unitPlanId: unit1.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: human body systems', 'PE: body awareness', 'French: body vocabulary']),
      vocabulary: JSON.stringify(['corps', 'tête', 'bras', 'jambes', 'cœur', 'poumons', 'muscles', 'spécial', 'différent', 'respecter']),
      homeConnection: 'Body appreciation with family: Share what makes each family member\'s body special and amazing.',
      socialEmotionalLearning: 'Body awareness, self-acceptance, respect for diversity, positive body image',
      isSubFriendly: true,
      unitPlanId: unit1.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: germs and health', 'Math: time and routines', 'French: self-care vocabulary']),
      vocabulary: JSON.stringify(['hygiène', 'routine', 'brosser', 'laver', 'sommeil', 'santé', 'soin', 'quotidien', 'habitude', 'objectif']),
      homeConnection: 'Family self-care plan: Work with family to create morning and evening self-care routines.',
      socialEmotionalLearning: 'Self-care, responsibility, goal-setting, healthy habits formation',
      isSubFriendly: true,
      unitPlanId: unit1.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: human development', 'Math: measurement and comparison', 'Arts: self-expression']),
      vocabulary: JSON.stringify(['grandir', 'changement', 'développement', 'progrès', 'compétence', 'apprentissage', 'célébrer', 'fier', 'capable']),
      homeConnection: 'Family growth stories: Ask family members to share how you\'ve grown and changed since you were little.',
      socialEmotionalLearning: 'Growth mindset, self-awareness, positive identity, resilience',
      isSubFriendly: true,
      unitPlanId: unit1.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Arts: talent expression', 'PE: physical strengths', 'French: strength vocabulary']),
      vocabulary: JSON.stringify(['force', 'talent', 'capacité', 'don', 'bon', 'apprendre', 'essayer', 'spécial', 'unique', 'fier']),
      homeConnection: 'Family strength stories: Ask family members what strengths they see in you and share your strengths star.',
      socialEmotionalLearning: 'Self-awareness, confidence building, appreciation of diversity, positive identity',
      isSubFriendly: true,
      unitPlanId: unit1.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Arts: color and emotion', 'Science: weather patterns', 'French: emotion vocabulary']),
      vocabulary: JSON.stringify(['émotion', 'arc-en-ciel', 'couleur', 'sentiment', 'météo', 'changer', 'valide', 'spectre', 'gamme', 'exprimer']),
      homeConnection: 'Family feelings rainbow: Create a feelings rainbow with family members. Discuss how families can support each other\'s emotions.',
      socialEmotionalLearning: 'Emotional vocabulary, emotion validation, emotional intelligence, self-expression',
      isSubFriendly: true,
      unitPlanId: unit2.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: thermometer concept', 'Math: counting strategies', 'French: emotion vocabulary']),
      vocabulary: JSON.stringify(['colère', 'fâché', 'thermomètre', 'stratégie', 'respirer', 'compter', 'calme', 'aide', 'normal', 'sécuritaire']),
      homeConnection: 'Family anger management: Share anger toolkit with family. Practice family anger management strategies.',
      socialEmotionalLearning: 'Anger management, self-regulation, emotional intelligence, conflict resolution',
      isSubFriendly: true,
      unitPlanId: unit2.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: weather and clouds', 'Arts: creative expression', 'French: worry vocabulary']),
      vocabulary: JSON.stringify(['inquiétude', 'anxiété', 'guerrier', 'stratégie', 'positif', 'problème', 'solution', 'aide', 'gérer', 'capable']),
      homeConnection: 'Family worry support: Share worry-busting strategies with family. Create family worry support plan.',
      socialEmotionalLearning: 'Anxiety management, resilience, problem-solving, emotional regulation',
      isSubFriendly: true,
      unitPlanId: unit2.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Arts: joyful expression', 'Science: recipe mixing', 'French: gratitude vocabulary']),
      vocabulary: JSON.stringify(['joie', 'bonheur', 'gratitude', 'reconnaissant', 'recette', 'ingrédient', 'célébrer', 'positif', 'heureux', 'partager']),
      homeConnection: 'Family gratitude practice: Share happiness recipe with family. Start family gratitude tradition.',
      socialEmotionalLearning: 'Gratitude, positive emotions, joy building, emotional well-being',
      isSubFriendly: true,
      unitPlanId: unit2.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: weather patterns', 'Arts: emotional expression', 'French: emotion vocabulary']),
      vocabulary: JSON.stringify(['tristesse', 'triste', 'normal', 'correct', 'réconfort', 'aide', 'activité', 'mieux', 'accepter', 'sain']),
      homeConnection: 'Family comfort plan: Share comfort toolkit with family. Practice family comfort strategies.',
      socialEmotionalLearning: 'Sadness acceptance, emotional validation, coping strategies, help-seeking',
      isSubFriendly: true,
      unitPlanId: unit2.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Arts: costume creation', 'Drama: superhero role-play', 'French: superhero vocabulary']),
      vocabulary: JSON.stringify(['super-pouvoir', 'héros', 'intelligence', 'émotionnelle', 'maîtriser', 'efficace', 'mission', 'aider', 'capacité', 'fort']),
      homeConnection: 'Family emotion superhero: Share emotion superhero identity with family. Use superpowers at home.',
      socialEmotionalLearning: 'Emotional mastery, self-efficacy, helping others, emotional leadership',
      isSubFriendly: true,
      unitPlanId: unit2.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Social Studies: family and community', 'Arts: family portraits', 'French: family vocabulary']),
      vocabulary: JSON.stringify(['famille', 'amour', 'soin', 'tradition', 'structure', 'membre', 'aider', 'spécial', 'différent', 'célébrer']),
      homeConnection: 'Family tradition exploration: Ask family about special traditions and create new family appreciation ritual.',
      socialEmotionalLearning: 'Family appreciation, diversity acceptance, belonging, relationship understanding',
      isSubFriendly: true,
      unitPlanId: unit4.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Math: recipe measurements', 'Arts: recipe book creation', 'French: friendship vocabulary']),
      vocabulary: JSON.stringify(['amitié', 'ami', 'recette', 'ingrédient', 'gentillesse', 'partager', 'écouter', 'bon', 'compétence', 'engagement']),
      homeConnection: 'Family friendship stories: Ask family about their best friends and how they maintain friendships.',
      socialEmotionalLearning: 'Friendship skills, kindness, empathy, social competence',
      isSubFriendly: true,
      unitPlanId: unit4.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Social Studies: community cooperation', 'Math: problem-solving strategies', 'French: peace vocabulary']),
      vocabulary: JSON.stringify(['conflit', 'problème', 'résoudre', 'paix', 'pacifique', 'écouter', 'parler', 'solution', 'ensemble', 'accord']),
      homeConnection: 'Family peace practice: Share peace steps with family. Practice peaceful problem-solving at home.',
      socialEmotionalLearning: 'Conflict resolution, peaceful communication, cooperation, problem-solving',
      isSubFriendly: true,
      unitPlanId: unit4.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: human body', 'Social Studies: rights and responsibilities', 'French: body safety vocabulary']),
      vocabulary: JSON.stringify(['corps', 'appartenir', 'privé', 'frontière', 'consentement', 'arrêter', 'non', 'sécurité', 'confiance', 'adulte']),
      homeConnection: 'Family body safety discussion: Parent communication sent home with discussion guide for body safety at home.',
      socialEmotionalLearning: 'Body autonomy, assertiveness, safety awareness, trust building',
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: body signals', 'Social Studies: personal rights', 'French: safety vocabulary']),
      vocabulary: JSON.stringify(['toucher', 'sécuritaire', 'non sécuritaire', 'confortable', 'inconfortable', 'signal', 'avertissement', 'réponse', 'confiance', 'aide']),
      homeConnection: 'Family safety reinforcement: Parent materials provided for continuing body safety education at home.',
      socialEmotionalLearning: 'Body awareness, assertiveness, safety skills, help-seeking behavior',
      isSubFriendly: false, // Requires specific teacher training
      unitPlanId: unit3.id,
      userId: emily.id
    });
    ...
    lessons.push({
      ...
      assessmentType: 'formative',
      crossCurricular: JSON.stringify(['Science: nutrition and health', 'Math: food counting and measuring', 'French: food vocabulary']),
      vocabulary: JSON.stringify(['nutrition', 'groupe alimentaire', 'fruits', 'légumes', 'grains', 'protéines', 'équilibré', 'variété', 'sain', 'repas']),
      homeConnection: 'Family nutrition exploration: Plan a colorful family meal together using foods from all groups.',
      socialEmotionalLearning: 'Health consciousness, goal setting, family cooperation, self-care',
      isSubFriendly: true,
    ...
    lessons.push({
      ...
      crossCurricular: JSON.stringify(['All subjects: integrated health learning', 'Arts: celebration creation', 'French: reflection vocabulary']),
      vocabulary: JSON.stringify(['célébration', 'apprentissage', 'héros', 'santé', 'bien-être', 'croissance', 'fier', 'objectif', 'continuer', 'engagement']),
      homeConnection: 'Family health celebration: Share health hero portfolio with family. Plan family summer wellness activities.',
      socialEmotionalLearning: 'Self-reflection, goal setting, celebration, peer appreciation, continued growth',
      isSubFriendly: true,
