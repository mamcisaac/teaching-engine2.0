...
exports.unitPlanProtectionMiddleware = void 0;
exports.addProtectedUnitPlan = addProtectedUnitPlan;
exports.removeProtectedUnitPlan = removeProtectedUnitPlan;
exports.getProtectedUnitPlans = getProtectedUnitPlans;
exports.isUnitPlanProtected = isUnitPlanProtected;
exports.bulkLockUnitPlans = bulkLockUnitPlans;
// List of unit plan IDs that are protected (will be populated after locking)
const PROTECTED_UNIT_PLAN_IDS = new Set();
// Error messages for different protection scenarios
const PROTECTION_ERRORS = {
    LOCKED_UNIT: (id) => `🔒 PROTECTED: Unit plan ${id} is locked and cannot be modified. ` +
        `See UNIT_PLANS_PROTECTION_PROTOCOL.md for override procedures.`,
    CANNOT_UNLOCK: (id) => `🔒 SECURITY: Cannot remove protection from unit plan ${id}. ` +
        `This requires administrative approval.`,
    BULK_PROTECTION_VIOLATION: () => `🔒 BULK PROTECTION: Cannot perform bulk operations on protected unit plans. ` +
 ...
 */
const unitPlanProtectionMiddleware = async (params, next) => {
    ...
    try {
        ...
        if (['update', 'upsert', 'delete'].includes(action)) {
            ...
            if (unitId) {
                ...
                const unit = await params.runInThisContext(() => next({
                    model: 'UnitPlan',
                    action: 'findUnique',
                    args: {
                        where: { id: unitId },
        ...
        if (['updateMany', 'deleteMany'].includes(action)) {
            ...
            const affectedUnits = await params.runInThisContext(() => next({
                model: 'UnitPlan',
                action: 'findMany',
                args: {
                    where: whereClause,
            ...
            const lockedUnits = affectedUnits.filter((unit) => unit.isLocked);
            if (lockedUnits.length > 0) {
                const lockedIds = lockedUnits.map((unit) => unit.id).join(', ');
...
exports.unitPlanProtectionMiddleware = unitPlanProtectionMiddleware;
/**
 * Add a unit plan ID to the protection list
 */
function addProtectedUnitPlan(unitId) {
 ...
 */
function removeProtectedUnitPlan(unitId) {
...
exports.default = exports.unitPlanProtectionMiddleware;
//# sourceMappingURL=unit-plan-protection.js.map