...
async function revolutionaryFrenchLanguageArtsSafe() {
  try {
    ...
    // PHASE 1: Safe removal of old system
    const currentUnits = await prisma.unitPlan.findMany({
      where: { longRangePlanId: frenchLRP.id },
      select: { id: true, title: true }
    });
    
    console.log(`🗑️  Safely removing ${currentUnits.length} units from old system...`);
    const unitIds = currentUnits.map(u => u.id);
    
    // Delete all foreign key dependencies first
    console.log('  Step 1: Removing lesson plans...');
    const lessonCount = await prisma.eTFOLessonPlan.deleteMany({
      where: { unitPlanId: { in: unitIds } }
    });
    console.log(`    Deleted ${lessonCount.count} lesson plans`);
    
    console.log('  Step 2: Removing unit expectations...');
    const expectationCount = await prisma.unitPlanExpectation.deleteMany({
      where: { unitPlanId: { in: unitIds } }
    });
    console.log(`    Deleted ${expectationCount.count} expectations`);
    
    console.log('  Step 3: Removing unit resources...');
    const resourceCount = await prisma.unitPlanResource.deleteMany({
      where: { unitPlanId: { in: unitIds } }
    });
    console.log(`    Deleted ${resourceCount.count} resources`);
    
    console.log('  Step 4: Removing transfer skills...');
    const transferSkillCount = await prisma.unitPlanTransferSkill.deleteMany({
      where: { unitPlanId: { in: unitIds } }
    });
    console.log(`    Deleted ${transferSkillCount.count} transfer skills`);
    
    console.log('  Step 5: Removing unit plans...');
    ...
    const frenchLanguageArtsUnits: FrenchLanguageArtsUnit[] = [
      {
        ...
        lessons: 23,
        estimatedHours: 17, // Rounded to integer for database
    ...
    console.log(`  Total Hours: ${totalHours} (target: ~146)`);
    ...
    console.log(`  ✅ Total Hours: ${finalTotal} (target: ~146)`);
...
revolutionaryFrenchLanguageArtsSafe();