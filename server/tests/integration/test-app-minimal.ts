/**
 * Minimal Test Application Setup for Integration Tests
 *
 * This provides a clean Express app instance for integration tests
 * with only the necessary routes and middleware for authentication testing
 */
...
app.get('/api/user/profile', authMiddleware(prisma), async (req, res) => {
  const userId = (req as any).userId;
  const user = await prisma.user.findUnique({
    where: { id: parseInt(userId) },
    select: {
