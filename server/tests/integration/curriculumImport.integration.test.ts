...
import { getTestAuthToken, testUserData } from '../test-auth-helper';
import fs from 'fs';
...
describe('Curriculum Import API Integration', () => {
  ...
  describe('GET /api/curriculum/import/:id/progress', () => {
    it('should return import progress', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body).toHaveProperty('status', ImportStatus.PROCESSING);
  ...
  describe('CSV Import', () => {
    it('should parse CSV content correctly', async () => {
      const { CurriculumImportService } = await import(
...
L1.1,"Listen to understand and respond appropriately",Language,1,Oral Communication`;
      ...
      const outcomes = service.parseCSV(csvContent);
