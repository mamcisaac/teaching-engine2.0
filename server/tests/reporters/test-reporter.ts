...
export default class TeachingEngineTestReporter implements Reporter {
  ...
  onTestResult(test: Test, testResult: TestResult): void {
    ...
    testResults.forEach((result) => {
      ...
      if (testResult.console && this.options.includeConsoleOutput) {
        const retryMatch = testResult.console.find(
          (log) => log.message.includes('passed after') && log.message.includes('retries'),
        );
        if (retryMatch) {
    ...
    console.log(
      `${icon} ${path.basename(testFilePath)}: ${passed} passed, ${failed} failed, ${skipped} skipped`,
    );
  }
  ...
  private async writeReport(summary: unknown): Promise<void> {
  ...
  private generateHtmlReport(summary: unknown): string {
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <table>
        ...
        ${summary.performance.slowestTests
          .map(
            (t) => `
            <tr>
                <td>${t.name}</td>
  ...
  private generateJUnitXml(summary: unknown): string {
...
<testsuites name="${this.escapeXml(summary.title)}" tests="${summary.totals.tests}" failures="${summary.totals.failed}" skipped="${summary.totals.skipped}" time="${(summary.duration / 1000).toFixed(3)}">
  ...
  private printSummary(summary: unknown): void {
    console.log('\n' + '='.repeat(80));
    ...
    if (summary.totals.failed > 0) {
      ...
      console.log('\n❌ Failed Tests:');
      summary.failures.forEach((f: unknown) => {
        console.log(`   - ${f.name}`);
        console.log(`     ${f.path}`);
    ...
    if (summary.flaky.length > 0) {
      ...
      summary.flaky.forEach((f: unknown) => {
    ...
    if (summary.performance.slowestTests.length > 0) {
      console.log(`\n🐌 Slowest tests:`);
      summary.performance.slowestTests.slice(0, 5).forEach((t: unknown) => {
