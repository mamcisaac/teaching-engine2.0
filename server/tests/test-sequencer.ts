...
export default class CustomTestSequencer extends Sequencer {
  ...
  shard(tests: Array<Test>, options: { shardIndex: number; shardCount: number }): Array<Test> {
    ...
    const shardedGroups: Test[][] = Array(shardCount)
      .fill(null)
      .map(() => []);
    ...
    const sortedGroupIndices = Array.from({ length: testGroups.length }, (_, i) => i).sort(
      (a, b) => groupDurations[b] - groupDurations[a],
    );
  ...
  static updateTestHistory(testPath: string, duration: number, failed: boolean): void {
    ...
    // Update failure rate
    const attempts = (existing as unknown).attempts || 0;
    const failures = (existing as unknown).failures || 0;
