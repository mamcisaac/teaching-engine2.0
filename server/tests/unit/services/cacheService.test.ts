...
describe('CacheService', () => {
  ...
  describe('performance and high load', () => {
    it('should handle high concurrent operations efficiently', async () => {
      ...
      for (let i = 0; i < concurrentOps; i++) {
        if (i % 3 === 0) {
          // Write operation
          operations.push(cacheService.set(`key-${i}`, `value-${i}`, { ttl: 60000 }));
        } else if (i % 3 === 1) {
          // Read operation
          operations.push(cacheService.get(`key-${Math.floor(i / 2)}`));
        } else {
          // getOrSet operation
          operations.push(
            cacheService.getOrSet(`computed-${i}`, async () => `computed-value-${i}`, {
              ttl: 60000,
            }),
          );
        }
      ...
      expect(endTime - startTime).toBeLessThan(1000); // Less than 1 second for 1000 ops
    ...
    it('should maintain memory efficiency under load', async () => {
      const testCache = new CacheService({ maxSize: 1000, defaultTTL: 60000 });

      // Fill cache to capacity
      for (let i = 0; i < 1500; i++) {
        await testCache.set(`memory-test-${i}`, { data: 'x'.repeat(1000) });
      }

      // Check that eviction kept size within limits
      const stats = testCache.getStats();
      expect(stats.size).toBeLessThanOrEqual(1000);
      expect(stats.memoryUsage).toBeGreaterThan(0);

      // Verify LRU eviction worked correctly
      // Early items should be evicted (we evict 10% when at capacity)
      expect(await testCache.get('memory-test-0')).toBeNull();
      expect(await testCache.get('memory-test-100')).toBeNull();
      // Recent items should exist
      expect(await testCache.get('memory-test-1499')).toBeTruthy();

      (testCache as unknown as { destroy: () => void }).destroy();
    });

    it('should handle rapid key updates without data corruption', async () => {
      ...
      expect(finalValue).toBe(updateCount - 1);
    });
  });

  describe('thread safety simulation', () => {
    it('should handle concurrent access to same key safely', async () => {
      ...
      const increments = Array(iterations)
        .fill(0)
        .map(async () => {
          const current = (await cacheService.get<number>(key)) || 0;
          await cacheService.set(key, current + 1);
        });

      await Promise.all(increments);

      // Due to race conditions, final value might not be exactly iterations
      // But it should be a valid number and not corrupted
      const finalValue = await cacheService.get<number>(key);
      expect(typeof finalValue).toBe('number');
      expect(finalValue).toBeGreaterThan(0);
      expect(finalValue).toBeLessThanOrEqual(iterations);
    });
  });

  describe('error handling and edge cases', () => {
    it('should handle circular references in cached objects gracefully', async () => {
      const circular: any = { a: 1 };
      circular.self = circular;

      // Should handle circular reference in size estimation
      await expect(cacheService.set('circular', circular)).resolves.not.toThrow();

      // Note: Retrieved value won't have circular reference due to JSON serialization in size estimation
      const retrieved = await cacheService.get('circular');
      expect(retrieved).toBeTruthy();
    });

    it('should handle extremely large keys gracefully', async () => {
      const largeKey = 'x'.repeat(10000);
      const value = 'test-value';

      await cacheService.set(largeKey, value);
      const retrieved = await cacheService.get(largeKey);
    ...
    it('should handle null and undefined values', async () => {
      ...
      // null should be stored and retrieved
      const nullValue = await cacheService.get('null-value');
      expect(nullValue).toBeNull();

      // undefined should be stored and retrieved
      const undefinedValue = await cacheService.get('undefined-value');
      expect(undefinedValue).toBeUndefined();
    });
  });

  describe('memory usage estimation', () => {
    it('should accurately estimate memory for different data types', () => {
      const cache = cacheService as unknown as {
        estimateValueSize: (value: unknown) => number;
      };

      // String estimation
      expect(cache.estimateValueSize('hello')).toBe(10); // 5 chars * 2 bytes

      // Number estimation
      expect(cache.estimateValueSize(42)).toBe(8);

      // Boolean estimation
      expect(cache.estimateValueSize(true)).toBe(1);

      // Object estimation
      const obj = { name: 'test', value: 123 };
      const estimatedSize = cache.estimateValueSize(obj);
      expect(estimatedSize).toBeGreaterThan(20); // Should account for JSON string size

      // Null/undefined estimation
      expect(cache.estimateValueSize(null)).toBe(0);
      expect(cache.estimateValueSize(undefined)).toBe(0);
    });
  });

  describe('production readiness', () => {
    it('should provide meaningful statistics for monitoring', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        if (i < 50) {
          await testCache.set(`prod-key-${i}`, `value-${i}`);
        }
        await testCache.get(`prod-key-${i}`); // 50 hits, 50 misses
      }
    ...
    it('should handle destroy/cleanup properly', () => {
      const testCache = new CacheService({ cleanupInterval: 100 });

      // Get reference to cleanup interval
      const cleanupInterval = (testCache as unknown as { cleanupInterval: NodeJS.Timeout })
        .cleanupInterval;
      ...
      // Note: We can't directly test if interval was cleared,
      // but calling destroy multiple times should not throw
      expect(() => (testCache as unknown as { destroy: () => void }).destroy()).not.toThrow();
    });
  });
});
