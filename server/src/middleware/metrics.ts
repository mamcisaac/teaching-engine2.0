...
class MetricsStore {
  ...
  private readonly defaultBuckets = [1, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000, Infinity];
 ...
 */
export function recordDatabaseQuery<T>(
  operation: string,
  queryFn: () => Promise<T>
): Promise<T> {
  const startTime = performance.now();
 ...
 */
export function calculatePercentiles(histogramData: HistogramData, percentiles: number[] = [50, 90, 95, 99]) {
  ...
  for (const p of percentiles) {
    ...
    for (const bucket of histogramData.buckets) {
      ...
      if (cumulativeCount >= targetCount) {
        result[`p${p}`] = bucket.le === Infinity ? histogramData.sum / histogramData.count : bucket.le;
        break;
      }
    }
  }
...
export function getPerformanceSummary() {
  ...
  const summary = {
    http: {
      ...
      errorRate: 0,
      responseTime: calculatePercentiles(metrics.histograms['http_request_duration_ms'] || { buckets: [], sum: 0, count: 0 })
    },
    database: {
      totalQueries: metrics.counters['db_queries_total'] || 0,
      totalErrors: metrics.counters['db_errors_total'] || 0,
      errorRate: 0,
      queryTime: calculatePercentiles(metrics.histograms['db_query_duration_ms'] || { buckets: [], sum: 0, count: 0 })
    },
    cache: {
  ...
  if (summary.database.totalQueries > 0) {
    summary.database.errorRate = (summary.database.totalErrors / summary.database.totalQueries) * 100;
  }
