...
export const errorContextMiddleware = (
  ...
  req.id = req.id || (req.headers['x-request-id'] !== null && req.headers['x-request-id'] !== undefined && req.headers['x-request-id'] !== '' ? req.headers['x-request-id'] as string : '') || uuidv4();
  ...
  res.on('finish', () => {
    const duration = req.startTime !== null && req.startTime !== undefined ? Date.now() - req.startTime : 0;
