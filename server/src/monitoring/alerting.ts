...
const alerts: Alert[] = [
  ...
  },
  {
    ...
    condition: async () => {
      const metrics = getMetrics();
      const p95 = metrics.http_request_duration_ms?.getPercentiles()?.p95 || 0;
  ...
  },
  {
    ...
    condition: async () => {
      const metrics = getMetrics();
      const errors = metrics.ai_operation_errors_total?.getValue() || 0;
      const total = metrics.ai_operations_total?.getValue() || 1;
      const errorRate = (errors / total) * 100;
      return errorRate > 20;
    },
    message: (context) => `AI operation error rate is ${context.errorRate.toFixed(2)}% (threshold: 20%)`,
    severity: 'warning',
    cooldown: 30,
    channels: ['log', 'email'],
  },
