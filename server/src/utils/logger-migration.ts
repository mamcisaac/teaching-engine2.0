...
import { structuredLogger, LogMeta } from './structuredLogger';
...
export function createCompatibleLogger(): LegacyLogger {
  return {
    ...
    log(level: string, message: string, ...args: any[]) {
      const [data, meta] = parseArgs(args);
      structuredLogger.log(level as any, message, { ...meta, data });
    },
  };
}
 ...
 */
function parseArgs(args: any[]): [any, LogMeta?] {
  if (args.length === 0) {
...
export class StructuredLoggerTransport extends winston.transports.Stream {
  ...
  log(info: any, callback: Function) {
    ...
    structuredLogger.log(level as any, message, meta);
...
export function migrateLogStatement(oldStatement: string): string {
  ...
  if (oldStatement.match(/logger\.(info|warn|debug)\s*\(\s*['"`]([^'"`]+)['"`]\s*,\s*([^)]+)\s*\)/)) {
    return oldStatement.replace(
 ...
 */
export function requestLoggerMiddleware(req: any, res: any, next: Function) {
...
export { structuredLogger, correlationMiddleware, errorLoggingMiddleware } from './structuredLogger';