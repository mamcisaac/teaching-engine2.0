...
export function validateEmail(email: string): EmailValidationResult {
  ...
  if (localPart !== undefined && localPart.length > 64) {
  ...
  if (domain !== undefined && domain.includes('..')) {
...
export function validateContact(contact: {
  ...
  if ((contact.phone === undefined || contact.phone === '') && (contact.email === undefined || contact.email === '')) {
    errors.push('Either phone number or email is required');
  }

  return {
    isValid: errors.length === 0,
    name: contact.name !== undefined && contact.name !== '' ? contact.name.trim() : undefined,
    phone: phoneResult,
    email: emailResult,
...
export function validateEmergencyContact(contact: {
  ...
  const errors = baseValidation.errors !== undefined ? [...baseValidation.errors] : [];

  // Additional validation for emergency contacts
  if (contact.relationship !== undefined && contact.relationship !== '' && contact.relationship.trim().length > 100) {
    errors.push('Relationship description too long');
  }

  if (contact.availability !== undefined && contact.availability !== '' && contact.availability.trim().length > 200) {
  ...
  return {
    ...baseValidation,
    isValid: errors.length === 0,
    relationship: contact.relationship !== undefined && contact.relationship !== '' ? contact.relationship.trim() : undefined,
    availability: contact.availability !== undefined && contact.availability !== '' ? contact.availability.trim() : undefined,
    errors: errors.length > 0 ? errors : undefined,
  };
}
