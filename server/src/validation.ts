...
export const milestoneCreateSchema = baseMilestoneSchema.refine(
  (data) => !data.startDate || data.endDate === undefined || new Date(data.startDate) <= new Date(data.endDate),
  {
...
export const milestoneUpdateSchema = baseMilestoneSchema
  ...
  .refine(
    (data) =>
      !data.startDate || data.endDate === undefined || new Date(data.startDate) <= new Date(data.endDate),
    {
      message: 'End date must be after or equal to start date',
      path: ['endDate'],
    },
