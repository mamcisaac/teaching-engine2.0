...
export class ClusteringService extends BaseService {
  ...
  async clusterOutcomes(
    ...
    try {
      ...
      if (outcomes.length < opts.minClusterSize) {
        this.logger.info({ importId, outcomeCount: outcomes.length }, 
          'Not enough outcomes to create meaningful clusters');
        return [];
      }

      this.logger.info({ importId, outcomeCount: outcomes.length, options: opts }, 
      ...
      this.logger.info({ importId, clusterCount: savedClusters.length }, 
    ...
    for (const outcome of outcomes) {
      ...
      for (const other of outcomes) {
        if (outcome.id !== other.id) {
          const similarity = embeddingService.calculateSimilarity(outcome.embedding, other.embedding);
      ...
      if (cluster.length >= options.minClusterSize) {
        ...
        for (let i = 0; i < cluster.length; i++) {
          for (let j = i + 1; j < cluster.length; j++) {
            ...
            totalSimilarity += embeddingService.calculateSimilarity(outcome1.embedding, outcome2.embedding);
            pairCount++;
          }
        }
        ...
        const clusterType = this.determineClusterType(cluster.map(id => 
  ...
  private determineClusterType(
    ...
    if (descriptions.includes('skill') || descriptions.includes('ability') || descriptions.includes('can')) {
      return 'skill';
    }
    
    if (descriptions.includes('concept') || descriptions.includes('understand') || descriptions.includes('knowledge')) {
      return 'concept';
    }
  ...
  private async generateClusterThemes(clusters: {
    name: string;
    type: 'theme' | 'skill' | 'concept';
    outcomeIds: string[];
    confidence: number;
...
Theme name:`;
        ...
        this.logger.error({ error, clusterId: cluster.outcomeIds }, 'Failed to generate theme for cluster');
      }
    }
  }
      ...
      try {
        const saved = await this.prisma.outcomeCluster.create({
          data: {
            ...
            confidence: cluster.confidence,
            suggestedTheme: cluster.name !== `Cluster ${clusters.indexOf(cluster) + 1}` ? cluster.name : undefined
          }
        });
