...
export class OERConnector extends BaseConnector {
  ...
  async search(params: SearchParams): Promise<ExternalActivity[]> {
    ...
    try {
      ...
      const activities: ExternalActivity[] = data.results.map((item: any) => 
        this.transformOERResource(item, params)
  ...
  private transformOERResource(resource: any, params: SearchParams): Omit<ExternalActivity, 'id' | 'createdAt' | 'updatedAt'> | null {
    ...
    return this.transformToExternalActivity({}, {
      ...
      learningGoals: resource.learning_objectives || [],
      isFree: true,
  ...
  private extractTechnologyRequirements(resource: any): string[] | null {
    const tech: string[] = [];
  ...
  private extractStandardsFromOER(standards: any[]): string[] {
