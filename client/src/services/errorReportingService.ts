...
export class ErrorReportingService {
    ...
    try {
      Sentry.init({
        ...
        beforeSendTransaction: (transaction) => {
          ...
          if (transaction.transaction && (transaction.transaction.includes('/static/') || transaction.transaction.includes('/assets/'))) {
            return null;
          }
          return transaction;
        },
  ...
  private beforeBreadcrumb(
    ...
    if (breadcrumb.category === 'console' && breadcrumb.level === 'warning') {
      ...
      if (message && (message.includes('DevTools') || message.includes('React Hook') || message.includes('StrictMode'))) {
        return null;
      }
