...
function localeMonths(m, format2) {
  ...
  return isArray(this._months)
    ...
    : this._months[
        (this._months.isFormat || MONTHS_IN_FORMAT).test(format2) ? 'format' : 'standalone'
      ][m.month()];
}
function localeMonthsShort(m, format2) {
  ...
  return isArray(this._monthsShort)
    ...
    : this._monthsShort[MONTHS_IN_FORMAT.test(format2) ? 'format' : 'standalone'][m.month()];
}
...
function localeWeekdays(m, format2) {
  var weekdays = isArray(this._weekdays)
    ? this._weekdays
    : this._weekdays[
        m && m !== true && this._weekdays.isFormat.test(format2) ? 'format' : 'standalone'
      ];
...
function calendar$1(time, formats) {
  ...
  var now2 = time || createLocal(),
    ...
    output =
      formats &&
      (isFunction(formats[format2]) ? formats[format2].call(this, now2) : formats[format2]);
