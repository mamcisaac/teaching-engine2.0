...
const prisma_1 = require("../server/src/prisma");
async function fixWinterWondersUnit() {
    console.log('🔧 Fixing Winter Wonders Unit to 95%+ quality...\n');
    // Find the unit
    const winterUnit = await prisma_1.prisma.unitPlan.findFirst({
        where: {
            userId: 23,
    ...
    const existingLessons = await prisma_1.prisma.eTFOLessonPlan.findMany({
        where: {
    ...
    const createDifferentiation = (topic) => ({
    ...
    for (const lesson of existingLessons) {
        ...
        let vocabSet = winterVocabularyFr.science;
        ...
        await prisma_1.prisma.eTFOLessonPlan.update({
            where: { id: lesson.id },
            data: {
                differentiationStrategies: JSON.stringify(createDifferentiation(lesson.title.split(':')[0])),
                vocabularyFr: vocabSet.join(', '),
    ...
    const lesson23 = {
        userId: 23,
        unitPlanId: winterUnit.id,
        title: "Winter Safety Heroes: Preparing for Emergencies",
        lessonNumber: 23,
        date: new Date('2026-01-15'),
        duration: 45,
        subject: "Sciences de la nature",
        learningGoals: "Students will investigate: How do we stay safe in winter emergencies? What are the essential items for winter safety? Through hands-on exploration and scientific inquiry, students will form hypotheses, conduct investigations, and draw conclusions about winter safety preparedness.",
        curriculum: "Science 1.3.1, 1.3.2 - Seasonal changes and effects on living things",
        ...
        safetyConsiderations: winterSafetyProtocols.general,
        ...
        assessment: "Formative: ☐ Identifies emergency items ☐ Explains safety purposes ☐ Participates in investigations ☐ Records observations ☐ Demonstrates safety awareness",
        differentiationStrategies: JSON.stringify(createDifferentiation("Winter Safety")),
        ...
        communityConnections: "Invite local emergency services to discuss winter safety. Partner with Red Cross for emergency preparedness presentation.",
    };
    const lesson24 = {
        userId: 23,
        unitPlanId: winterUnit.id,
        title: "Winter Celebration: Our Learning Journey",
        lessonNumber: 24,
        date: new Date('2026-01-16'),
        duration: 45,
        subject: "Sciences de la nature",
        learningGoals: "Students will investigate: What have we learned about winter? How have living things adapted to winter? Through hands-on exploration and scientific inquiry, students will synthesize their learning, present investigations, and celebrate winter discoveries.",
        curriculum: "Science 1.3.1, 1.3.2, 1.1.1 - Seasonal changes, adaptations, living things",
        ...
        safetyConsiderations: winterSafetyProtocols.indoor,
        ...
        assessment: "Summative: ☐ Explains winter adaptations ☐ Demonstrates investigation skills ☐ Uses scientific vocabulary ☐ Shares learning with others ☐ Shows growth from unit beginning",
        differentiationStrategies: JSON.stringify(createDifferentiation("Winter Learning")),
        ...
        communityConnections: "Invite families to Winter Learning Showcase. Share class winter discoveries book with school library. Connect with senior center to share winter stories.",
    };
    console.log('\n📝 Creating missing lessons 23-24...');
    await prisma_1.prisma.eTFOLessonPlan.create({ data: lesson23 });
    console.log('✅ Created Lesson 23: Winter Safety Heroes');
    await prisma_1.prisma.eTFOLessonPlan.create({ data: lesson24 });
    console.log('✅ Created Lesson 24: Winter Celebration');
    // Final count
    const finalCount = await prisma_1.prisma.eTFOLessonPlan.count({
