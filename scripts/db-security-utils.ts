 ...
 */
export function isValidIdentifier(identifier: string, maxLength = 64, requireAllowlist = false): boolean {
 ...
 */
export function validateTableName(tableName: string, requireAllowlist = true): string {
 ...
 */
export function validateDatabaseName(dbName: string, requireAllowlist = true): string {
 ...
 */
export function createSafeDeleteStatement(tableName: string, requireAllowlist = true) {
  const validatedTableName = validateTableName(tableName, requireAllowlist);
  // Return a function that creates the Prisma raw query
  return (prisma: any) => prisma.$executeRaw`DELETE FROM ${prisma.Prisma.raw(`"${validatedTableName}"`)}`;
}
 ...
 * @returns A Prisma raw template literal for safe execution
 */
export function createSafeCreateDatabaseStatement(dbName: string, requireAllowlist = true) {
  const validatedDbName = validateDatabaseName(dbName, requireAllowlist);
  return (prisma: any) => prisma.$executeRaw`CREATE DATABASE ${prisma.Prisma.raw(`"${validatedDbName}"`)}`;
}
 ...
 * @returns A Prisma raw template literal for safe execution
 */
export function createSafeDropDatabaseStatement(dbName: string, requireAllowlist = true) {
  const validatedDbName = validateDatabaseName(dbName, requireAllowlist);
  return (prisma: any) => prisma.$executeRaw`DROP DATABASE IF EXISTS ${prisma.Prisma.raw(`"${validatedDbName}"`)}`;
}
 ...
 * @returns A Prisma raw template literal for safe execution
 */
export function createSafeAlterSequenceStatement(tableName: string, requireAllowlist = true) {
  const validatedTableName = validateTableName(tableName, requireAllowlist);
  return (prisma: any) => prisma.$executeRaw`ALTER SEQUENCE IF EXISTS ${prisma.Prisma.raw(`"${validatedTableName}_id_seq"`)} RESTART WITH 1`;
}
 ...
 */
export function createSafeTableIdentifier(tableName: string, requireAllowlist = true) {
  const validatedTableName = validateTableName(tableName, requireAllowlist);
  return (prisma: any) => prisma.Prisma.raw(`"${validatedTableName}"`);
}
 ...
 * @returns A Prisma.raw() wrapped identifier
 */
export function createSafeFieldIdentifier(fieldName: string, allowedFields?: string[]) {
  const validatedFieldName = validateFieldName(fieldName, allowedFields);
  return (prisma: any) => prisma.Prisma.raw(`"${validatedFieldName}"`);
}