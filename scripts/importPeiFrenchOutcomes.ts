...
async function main() {
  try {
    ...
    const pdfPath = path.resolve(__dirname, '../resources/PE_Grade1_Fr/PR 2766 - Prog. Immersion 1re ann̩e 5.30.19.pdf');
    ...
    outcomes.slice(0, 5).forEach((o, i) => {
      console.log(`${i+1}. ${o.code}: ${o.description.substring(0, 100)}${o.description.length > 100 ? '...' : ''}`);
    });
